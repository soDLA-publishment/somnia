...
val rd_data_mask = RegInit("b1".asUInt(conf.CACC_DWIDTH_DIV_PWIDTH.W))
val rd_data_mask_pre = if(conf.CACC_DWIDTH_DIV_PWIDTH>=2) 
                       Mux(io.cacc2ppu_pd.valid & io.cacc2ppu_pd.ready, 
                       Cat(rd_data_mask(conf.CACC_DWIDTH_DIV_PWIDTH-2, 0), rd_data_mask(conf.CACC_DWIDTH_DIV_PWIDTH-1)), 
                       rd_data_mask)
                       else
                       rd_data_mask
rd_data_mask := rd_data_mask_pre
val data_left_mask_pre = Mux(dbuf_rd_en_new, Fill(conf.CACC_DWIDTH_DIV_PWIDTH, true.B), 
                         Mux(io.cacc2ppu_pd.valid & io.cacc2ppu_pd.ready, data_left_mask<<1.U, data_left_mask))(conf.CACC_DWIDTH_DIV_PWIDTH-1, 0)
data_left_mask := data_left_mask_pre
io.cacc2ppu_pd.valid := data_left_mask.orR
io.dbuf_rd_ready := ~(data_left_mask.orR)

val cacc2ppu_pd_data = VecInit((0 to conf.CACC_DWIDTH_DIV_PWIDTH-1) map 
                      { i => dbuf_rd_data(conf.CACC_PPU_DATA_WIDTH*(i+1)-1, conf.CACC_PPU_DATA_WIDTH*i)&Fill(conf.CACC_PPU_DATA_WIDTH, rd_data_mask(i))}).reduce(_|_)
...
val cacc2ppu_batch_end = false.B
val cacc2ppu_layer_end = dbuf_rd_layer_end_latch & last_data & io.cacc2ppu_pd.valid & io.cacc2ppu_pd.ready; //data_left_mask=0;
io.cacc2ppu_pd.bits := Cat(cacc2ppu_layer_end, cacc2ppu_batch_end, cacc2ppu_pd_data)

// generate CACC done interrupt  
val intr_sel = RegInit(false.B)
val cacc_done_intr  = RegInit("b0".asUInt(2.W))
val cacc_done = io.cacc2ppu_pd.valid & io.cacc2ppu_pd.ready & cacc2ppu_layer_end
...
last_data := (data_left_mask === Cat(true.B, Fill(conf.CACC_DWIDTH_DIV_PWIDTH-1, false.B)));
io.accu2sc_credit_size.valid := RegNext(io.cacc2ppu_pd.valid & io.cacc2ppu_pd.ready & last_data, false.B)
