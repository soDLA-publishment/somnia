...
class NV_NVDLA_CSB_MASTER_for_client(address_space: String)(implicit conf: nvdlaConfig) extends Module {
    val io = IO(new Bundle {
        ...
        val csb2dp = Flipped(new csb2dp_if)
...
withClock(io.nvdla_core_clk){
    ...
    val csb2dp_req_pvld_out = RegInit(false.B)
    val csb2dp_req_pd_tmp = Reg(UInt(50.W))
    ...
    val client_req_pvld_w = Mux(io.core_req_pop_valid & io.select_client, true.B,
                            Mux(~io.csb2dp.req.valid, false.B,
                            client_req_pvld))
    val csb2dp_req_pvld_w = client_req_pvld
    val csb2dp_req_en = client_req_pvld & (~io.csb2dp.req.valid)    

    client_req_pvld := client_req_pvld_w
    csb2dp_req_pvld_out := client_req_pvld
    when(csb2dp_req_en){
        csb2dp_req_pd_tmp := io.core_req_pd_d1
    }
    io.csb2dp.req.bits := Cat("b0".asUInt(7.W), csb2dp_req_pd_tmp(49, 16), "b0".asUInt(6.W), csb2dp_req_pd_tmp(15, 0))
    client_resp_valid_out := io.csb2dp.resp.valid
    when(io.csb2dp.resp.valid){
        client_resp_pd_out := io.csb2dp.resp.bits
    }

    io.csb2dp.req.valid := csb2dp_req_pvld_out
